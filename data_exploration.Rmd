...
mdt.melt.industry$value<-as.factor(trimws(mdt.melt.industry$value))

mdt.melt.industry$value %>%
  .[,.SD[order(-N)]] %>%
  .[, head(.SD, 10),]%>%
  ggvis(x=~skills, y=~N) %>%
  layer_bars(fillOpacity := 0.1)%>%
...
Given that the postdate where probably the date where the job listing is posted, and that it is not shown whether the data is fully subsetted over time,

If we assume that this captured all the job listing over 2016 to 2017 (probably unlikely given the huge variance over the period and that there are months where there is little to no listing), it would seems that most jobs are posted in March in 2016 ( Start of year-ish), followed by Nov 2015 (End of year).
...
dt.location <-  melt(mdt,  measure.vars = patterns("^locationS"))
