...
```
Intestingly, there are job listing that have 2000 positions, a quick look at this outlier.
...
```{r}
mdt<-mdt[is.na(numberofpositions), numberofpositions:=1]
summary(mdt$numberofpositions)
```
...
```{r}
# tally the count of location with top 100 locations, 
...
if(any(filelist=="mydat.rds")){
  ...
  geocodes <- geocode(as.character(mdt.melt.location.count.t100$value))
...
textplot_wordcloud(dfm_select(dfm.txt, documents=popular_industry_count$industry[1:4]), 
                   comparison = T, rot.per = .25, scale=c(2,.35),title.size =2)
...
If we were to ignore the outliers (which would be heavily influenced by the amount of job listing in that particular industry, it seems that "Iron and Steel" , "IT-Software" and "Strategy/Management consulting Firm" command the best payrate. 

Some industries payrate are rather pathetic, this is exemplify by "Animation/Gaming", "Government/Defense", "Well/Fitness/Sports" and "NGO/Social Services". At this point, I am not sure if it is caused by high attrition rates (people just dont stay in these industry for long, they change) or that the pay is
...
tempt_ggplot <- filtered.IMP %>%
  ggplot(aes(x=factor(log(m.experience.Median)), y=log(m.payrate.Mean))) +  
  geom_boxplot() +
...
```{r}
mdt<-mdt[, m.experience.Cut := cut(m.experience.Median, breaks=c(-Inf, 3*(0:3), 10+4*(0:2), Inf), right=F)]
...
This section examine the payrate commanded by Gradautes and people with various years of working experience. 
...
Although I wonder if the differences are not shown clearly are due to the scaling on the y-axis.
...
Seeing that we have data on the wages and job listing  for IT - Software
