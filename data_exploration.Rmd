...
```{r}
# Removing str in parenthesis as the information is redundant for our purpose of data exploration
...
mdt.melt.location.t100<-mdt.melt.location  %>%
  .[!is.na(value),.N,by=list(value)]%>%
  .[,.SD[order(-N)]] %>%
  .[, head(.SD, 100),]
...
geocodes <- geocode(as.character(mdt.melt.location.t100$value))
mydat <- data.frame(mdt.melt.location.t100[,1:2],geocodes)

# The locations that have absurd lon and lat are to be removed
 mydat.filtered <- setDT(mydat)[lon>60&lon<80] %>%
   .[lat>10&lat<30] 
...
ggmap(india.map, extent = "panel", maprange=FALSE)+
   ...
   # stat_density2d(data = mydat, aes(x = lon, y = lat, fill = N, alpha = N),
...
# glimpse(mapdata)
# 
#  # mdt.melt.location.t100<- 
#  mdt.melt.location.t100<- mdt.melt.location  %>%
#    .[!is.na(value),list(.N), by=list(value)]%>%
#    .[,z:=N]%>%
#    .[,.SD[order(-N)]] %>%
#    .[, head(.SD, 100),]
#  mdt.melt.location.t100$color<- colorize(mdt.melt.location.t100$N)

is.data.table(mydat.filtered)
 
 # Setting z,color and name parameter for plotting with highcharter 
 highcharter.plot <- setDT(mydat)[lon>60&lon<80] %>%
   .[lat>10&lat<30]%>%
   .[,`:=`(
           name=value,
           z=N,
